...
export default function WhatsAppPage() {
    ...
    // Meta Cloud API Tab & Broadcast States
    const [activeChannelTab, setActiveChannelTab] = useState<'QR_BAILEYS' | 'META_CLOUD'>('QR_BAILEYS');
...
${recentContext}`;
    ...
    // Render Meta Cloud Tab View
    const renderMetaCloudTab = () => (
        <div className="flex-1 bg-gray-50 dark:bg-slate-950 p-6 md:p-8 overflow-y-auto">
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
                    <div className="flex items-center justify-between flex-wrap gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-xl font-bold text-gray-900 dark:text-slate-100">Resmi Meta WhatsApp Cloud API</h2>
                                <p className="text-sm text-gray-500 dark:text-slate-400">Aracısız, doğrudan Meta Graph API & Akıllı Rezervasyon Chatbot Entegrasyonu</p>
                            </div>
                        </div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-4">
                    ...
                    <p className="text-sm text-gray-600 dark:text-slate-300">
                        Meta Developer Panelinizde (developers.facebook.com) WhatsApp &gt; Configuration &gt; Webhook bölümüne aşağıdaki bilgileri tanımlayın:
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="p-3 bg-gray-50 dark:bg-slate-800/60 rounded-xl border border-gray-100 dark:border-slate-700/60">
                            ...
                            <code className="text-xs font-mono font-semibold text-blue-600 dark:text-blue-400 break-all select-all">
                                {typeof window !== 'undefined' ? `${window.location.origin}/api/whatsapp/cloud/webhook` : 'https://your-crm.vercel.app/api/whatsapp/cloud/webhook'}
                            </code>
                        </div>
                        <div className="p-3 bg-gray-50 dark:bg-slate-800/60 rounded-xl border border-gray-100 dark:border-slate-700/60">
                    ...
                    <div className="p-3 bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800/60 rounded-xl text-xs text-blue-800 dark:text-blue-300">
                        💡 <strong>Webhook İzinleri:</strong> Meta panelinde <code>messages</code> alanını &quot;Subscribe&quot; (Abone ol) yapmanız yeterlidir. Müşteri yazdığında chatbot anında devreye girecektir.
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-4">
                    <h3 className="text-base font-bold text-gray-900 dark:text-slate-100 flex items-center gap-2">
                        🤖 Akıllı Transfer Chatbot Akışı (Jasper&apos;s Market Mantığı)
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                        <div className="p-3 bg-gray-50 dark:bg-slate-800 rounded-xl">
                            <strong className="block text-gray-900 dark:text-slate-100 mb-1">1. Karşılama &amp; Menü</strong>
                            <p className="text-gray-500 dark:text-slate-400">Müşteri yazdığında otomatik interaktif butonlar sunulur (Havalimanı, Şehirlerarası vb.).</p>
                        </div>
                        <div className="p-3 bg-gray-50 dark:bg-slate-800 rounded-xl">
                            <strong className="block text-gray-900 dark:text-slate-100 mb-1">2. Güzergah &amp; Fiyat</strong>
                            <p className="text-gray-500 dark:text-slate-400">Açılır listeden rota seçilir, tarih/saat ve yolcu sayısı alınarak anlık fiyat hesaplanır.</p>
                        </div>
                        <div className="p-3 bg-gray-50 dark:bg-slate-800 rounded-xl">
                            <strong className="block text-gray-900 dark:text-slate-100 mb-1">3. Otomatik CRM Kaydı</strong>
                            <p className="text-gray-500 dark:text-slate-400">Müşteri onayladığı an Rezervasyonlar listesine Voucher numarasıyla eklenir ve Telegram bildirimi atılır.</p>
                        </div>
                    </div>
                </div>
    ...
    const renderChannelSwitcher = () => (
        <div className="bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 px-4 py-2.5 flex items-center justify-between shrink-0 shadow-sm">
            <div className="flex items-center gap-2">
                <button
                    onClick={() => setActiveChannelTab('QR_BAILEYS')}
                    className={`px-4 py-2 rounded-xl text-xs md:text-sm font-semibold transition-all flex items-center gap-2 ${
                        activeChannelTab === 'QR_BAILEYS'
                            ? 'bg-green-500 text-white shadow-sm'
                            : 'bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-700'
                    }`}
                >
                    📱 QR Kodlu WhatsApp (Kişisel / Web)
                </button>
                <button
                    onClick={() => setActiveChannelTab('META_CLOUD')}
                    className={`px-4 py-2 rounded-xl text-xs md:text-sm font-semibold transition-all flex items-center gap-2 ${
                        activeChannelTab === 'META_CLOUD'
                            ? 'bg-emerald-600 text-white shadow-sm'
                            : 'bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-700'
                    }`}
                >
                    ⚡ Resmi Meta Cloud API &amp; Chatbot
                </button>
            </div>
            <div className="hidden md:flex items-center gap-2 text-xs text-gray-500 dark:text-slate-400">
                <span>Durum:</span>
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                <span className="font-medium text-emerald-600 dark:text-emerald-400">Cloud API Hazır</span>
            </div>
        </div>
    );

    // ── Render: If META_CLOUD tab is active ────────────────────────────────────
    if (activeChannelTab === 'META_CLOUD') {
        return (
            <div className="h-full flex flex-col bg-gray-100 dark:bg-slate-950 overflow-hidden text-gray-900 dark:text-slate-100">
